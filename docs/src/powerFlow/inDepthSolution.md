...
JuliaGrid is based on common network elements and benefits the [unified branch model](@ref inDepthACModel) to find the power flow solution and perform the power flow analysis,  which is used for defining load profiles, generator capabilities, voltage specification, contingency analysis, and planning. At the beginning, JuliaGrid requires the composite type `PowerSystem`, which is obtained by using the function [`powerSystem()`](@ref powerSystem), for example:
```julia-repl
...
```
The complex injection current into the bus ``i \in \mathcal{N}`` is defined as:
```math
...
To solve the AC power flow and find the bus voltage magnitudes and angles using Newton-Raphson method, JuliaGrid provides the following sequence of functions:
```julia-repl
system = powerSystem("case14.h5")
acModel!(system)
...
#### Method Implementation
To solve the AC power flow and find the bus voltage magnitudes and angles using Gauss-Seidel method, JuliaGrid provides the following sequence of functions:
```julia-repl
system = powerSystem("case14.h5")
acModel!(system)
