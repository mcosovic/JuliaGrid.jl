...
addWattmeter!(system, device; from = "Branch 1", active = 0.2, variance = 1e-3)
```
...
The first ammeter's measurement value is created by adding white Gaussian noise with the `variance` to the `magnitude` value. In contrast, for the second ammeter, we assume that we already know the measurement value, defined by the `magnitude`. This is achieved by setting `noise = false`. Consequently, these actions yield the following results:
```@repl addAmmeter
...
##### Customizing Input Units for Keywords
By default, the `magnitude` and `variance` keywords are expected to be provided in per-unit (pu). However, users have the flexibility to express these values in amperes (A) if they prefer. Take a look at the following example for illustration:
...
If a user wishes to isolate only the wattmeters located at the buses, they can achieve this by using the following code:
```@repl retrievingLabels
label[device.wattmeter.layout.bus]
```
This approach can also be extended to branches by using the `from` and `to` variables instead of `bus`.

Moreover, the `bus`, `from`, and `to` keywords associated with bus and branch labels are stored using internally assigned numerical values. To retrieve these labels, you can follow the procedure we provided for the wattmeters located at the buses:
```@repl retrievingLabels
label = collect(keys(sort(system.bus.label; byvalue = true)));
index = device.wattmeter.layout.index[device.wattmeter.layout.bus];

label[index]
```

Furthermore, if users want to retrieve branch labels associated with wattmeters located at the "from" bus ends, we can use:
```@repl retrievingLabels
label = collect(keys(sort(system.branch.label; byvalue = true)));
index = device.wattmeter.layout.index[device.wattmeter.layout.from];
...
nothing  # hide
```
In this example, we add voltmeters to all buses, ammeters to all branches on both sides of each branch, wattmeters, varmeters, and PMUs to all buses and on both sides of each branch. It is important to note that JuliaGrid follows a specific order: it first adds bus measurements, then branch measurements. For branches, it adds measurements located at the "from" bus end, and immediately after, measurements at the "to" bus end. This process is repeated for all branches.
...
nothing  # hide
```
In this example, we make adjustments to the measurement and variance values of the ammeter located at `Branch 2`, specifically at the "from" bus end. Simultaneously, we deactivate the ammeter at the same branch on the "to" bus end.
