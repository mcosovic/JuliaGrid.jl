...
@testset "Reusing Meters AC State Estimation" begin
    ...
    for (key, value) in system14.bus.label
        if value == 1
            ...
            addPmu!(system14, device; bus = key, magnitude = analysis.voltage.magnitude[value], angle = rand(1)[])
        elseif value == 3
            addVoltmeter!(system14, device; bus = key, magnitude = analysis.voltage.magnitude[value], status = 0)
            addWattmeter!(system14, device; bus = key, active = analysis.power.injection.active[value], status = 0)
            addVarmeter!(system14, device; bus = key, reactive = analysis.power.injection.reactive[value], status = 0)
        ...
        elseif value == 5
            addVoltmeter!(system14, device; bus = key, magnitude = rand(1)[], variance = 1e-5, noise = true)
            addWattmeter!(system14, device; bus = key, active = rand(1)[], variance = 1e-5, noise = true)
            addVarmeter!(system14, device; bus = key, reactive = rand(1)[], variance = 1e-5, noise = true)
            addPmu!(system14, device; bus = key, magnitude = 1, angle = rand(1)[], varianceAngle = 1e-5, noise = true)
        else
    ...
    updateVoltmeter!(system14, device; label = 5, magnitude = analysis.voltage.magnitude[5], variance = 1e-2)

    updateWattmeter!(system14, device; label = 1, status = 0)
    updateWattmeter!(system14, device; label = 3, status = 1)
    updateWattmeter!(system14, device; label = 5, active = analysis.power.injection.active[5], variance = 1e-2)
    ...
    updateWattmeter!(system14, device; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)

    updateVarmeter!(system14, device; label = "From 4", status = 0)
    updateVarmeter!(system14, device; label = "From 15", status = 1)
    updateVarmeter!(system14, device; label = "From 17", reactive = analysis.power.from.reactive[17], variance = 1e-2)
    ...
    updateWattmeter!(system14, device; label = "To 17", active = analysis.power.to.active[17], variance = 1e-2)

    updateVarmeter!(system14, device; label = "To 4", status = 0)
    updateVarmeter!(system14, device; label = "To 15", status = 1)
    updateVarmeter!(system14, device; label = "To 17", reactive = analysis.power.to.reactive[17], variance = 1e-2)
    ...
    updateVoltmeter!(system14, device, analysisWLS; label = 5, magnitude = analysis.voltage.magnitude[5], variance = 1e-2)

    updateWattmeter!(system14, device, analysisWLS; label = 1, status = 0)
    updateWattmeter!(system14, device, analysisWLS; label = 3, status = 1)
    updateWattmeter!(system14, device, analysisWLS; label = 5, active = analysis.power.injection.active[5], variance = 1e-2)
    ...
    updateWattmeter!(system14, device, analysisWLS; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)
    ...
    updateVarmeter!(system14, device, analysisWLS; label = "From 17", reactive = analysis.power.from.reactive[17], variance = 1e-2)
    ...
    updateWattmeter!(system14, device, analysisWLS; label = "To 17", active = analysis.power.to.active[17], variance = 1e-2)
    ...
    updateVarmeter!(system14, device, analysisWLS; label = "To 17", reactive = analysis.power.to.reactive[17], variance = 1e-2)
    ...
    updateVoltmeter!(system14, device, analysisLAV; label = 5, magnitude = analysis.voltage.magnitude[5], variance = 1e-2)

    updateWattmeter!(system14, device, analysisLAV; label = 1, status = 0)
    updateWattmeter!(system14, device, analysisLAV; label = 3, status = 1)
    updateWattmeter!(system14, device, analysisLAV; label = 5, active = analysis.power.injection.active[5], variance = 1e-2)
    ...
    updateWattmeter!(system14, device, analysisLAV; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)
    ...
    updateVarmeter!(system14, device, analysisLAV; label = "From 17", reactive = analysis.power.from.reactive[17], variance = 1e-2)
    ...
    updateWattmeter!(system14, device, analysisLAV; label = "To 17", active = analysis.power.to.active[17], variance = 1e-2)
    ...
    updateVarmeter!(system14, device, analysisLAV; label = "To 17", reactive = analysis.power.to.reactive[17], variance = 1e-2)
...
@testset "Reusing Meters PMU State Estimation" begin
    ...
    for (key, value) in placement.bus
        if value == 1
            addPmu!(system14, device; bus = key, magnitude = rand(1)[], angle = analysis.voltage.angle[value])
        elseif value == 4
            addPmu!(system14, device; bus = key, magnitude = analysis.voltage.magnitude[value], angle = rand(1)[], statusMagnitude = 0, correlated = true)
        elseif value == 6
            addPmu!(system14, device; bus = key, magnitude = rand(1)[], angle = rand(1)[], statusAngle = 0)
        else
            addPmu!(system14, device; bus = key, magnitude = analysis.voltage.magnitude[value], angle = analysis.voltage.angle[value])
        end
    end
    ...
    for (key, value) in placement.from
        ...
        elseif value == 9
            addPmu!(system14, device; from = key, magnitude = analysis.current.from.magnitude[value], angle = analysis.current.from.angle[value], statusAngle = 0, statusMagnitude = 0)
        elseif value == 12
            addPmu!(system14, device; from = key, magnitude = analysis.current.from.magnitude[value], angle = rand(1)[], correlated = true)
        else
            addPmu!(system14, device; from = key, magnitude = analysis.current.from.magnitude[value], angle = analysis.current.from.angle[value])
        end
    end
    for (key, value) in placement.to
        if value == 4
            addPmu!(system14, device; to = key, magnitude = rand(1)[], angle = analysis.current.to.angle[value])
        elseif value == 10
            addPmu!(system14, device; to = key, magnitude = analysis.current.to.magnitude[value], angle = rand(1)[])
        elseif value == 15
    ...
    updatePmu!(system14, device; label = 16,  magnitude = analysis.voltage.magnitude[6], angle = analysis.voltage.angle[6], statusAngle = 1)

    updatePmu!(system14, device; label = "From 8", magnitude = analysis.current.from.magnitude[8], angle = analysis.current.from.angle[8])
    ...
    #### Update Devices and Original LAV Model #######
    updatePmu!(system14, deviceLAV, analysisLAV; label = 1, magnitude = analysis.voltage.magnitude[1])
    updatePmu!(system14, deviceLAV, analysisLAV; label = 4, angle = analysis.voltage.angle[4], statusMagnitude = 1)
    updatePmu!(system14, deviceLAV, analysisLAV; label = 16,  magnitude = analysis.voltage.magnitude[6], angle = analysis.voltage.angle[6], statusAngle = 1)
    ...
    updatePmu!(system14, deviceWLS, analysisWLS; label = "From 12", angle = analysis.current.from.angle[12], correlated = true)
...
@testset "Reusing Meters DC State Estimation" begin
    ...
    for (key, value) in system14.bus.label
        ...
        elseif value == 6
            addPmu!(system14, device; bus = key, magnitude = 1, angle = analysis.voltage.angle[value], statusAngle = 0)
        elseif value == 9
            addPmu!(system14, device; bus = key, magnitude = 1, angle = rand(1)[], varianceAngle = 1e-5, noise = true)
        elseif value == 13
            addPmu!(system14, device; bus = key, magnitude = 1, angle = rand(1)[])
        else
            addPmu!(system14, device; bus = key, magnitude = 1, angle = analysis.voltage.angle[value])
        end
    end
    ...
    updateWattmeter!(system14, device; label = "From 15", status = 1)
    updateWattmeter!(system14, device; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)
    updateWattmeter!(system14, device; label = "From 20", active = analysis.power.from.active[20])

    updateWattmeter!(system14, device; label = "To 5", status = 0)
    updateWattmeter!(system14, device; label = "To 8", status = 1)
    updateWattmeter!(system14, device; label = "To 11", active = analysis.power.to.active[11], variance = 1e-2)
    updateWattmeter!(system14, device; label = "To 19", active = analysis.power.to.active[19])

    updatePmu!(system14, device; label = 2, statusAngle = 0)
    updatePmu!(system14, device; label = 6, statusAngle = 1)
    updatePmu!(system14, device; label = 9, angle = analysis.voltage.angle[9], varianceAngle = 1e-5)
    ...
    updateWattmeter!(system14, device, analysisWLS; label = 3, status = 1)
    updateWattmeter!(system14, device, analysisWLS; label = 5, active = analysis.power.injection.active[5], variance = 1e-2)
    updateWattmeter!(system14, device, analysisWLS; label = 9, active = analysis.power.injection.active[9])

    updateWattmeter!(system14, device, analysisWLS; label = "From 4", status = 0)
    updateWattmeter!(system14, device, analysisWLS; label = "From 15", status = 1)
    updateWattmeter!(system14, device, analysisWLS; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)
    updateWattmeter!(system14, device, analysisWLS; label = "From 20", active = analysis.power.from.active[20])
    ...
    updateWattmeter!(system14, device, analysisWLS; label = "To 11", active = analysis.power.to.active[11], variance = 1e-2)
    updateWattmeter!(system14, device, analysisWLS; label = "To 19", active = analysis.power.to.active[19])

    updatePmu!(system14, device, analysisWLS; label = 2, statusAngle = 0)
    updatePmu!(system14, device, analysisWLS; label = 6, statusAngle = 1)
    updatePmu!(system14, device, analysisWLS; label = 9, angle = analysis.voltage.angle[9], varianceAngle = 1e-5)
    updatePmu!(system14, device, analysisWLS; label = 13, angle = analysis.voltage.angle[13])
    ...
    updateWattmeter!(system14, device, analysisLAV; label = 3, status = 1)
    updateWattmeter!(system14, device, analysisLAV; label = 5, active = analysis.power.injection.active[5], variance = 1e-2)
    updateWattmeter!(system14, device, analysisLAV; label = 9, active = analysis.power.injection.active[9])

    updateWattmeter!(system14, device, analysisLAV; label = "From 4", status = 0)
    updateWattmeter!(system14, device, analysisLAV; label = "From 15", status = 1)
    updateWattmeter!(system14, device, analysisLAV; label = "From 17", active = analysis.power.from.active[17], variance = 1e-2)
    updateWattmeter!(system14, device, analysisLAV; label = "From 20", active = analysis.power.from.active[20])
    ...
    updateWattmeter!(system14, device, analysisLAV; label = "To 11", active = analysis.power.to.active[11], variance = 1e-2)
    updateWattmeter!(system14, device, analysisLAV; label = "To 19", active = analysis.power.to.active[19])

    updatePmu!(system14, device, analysisLAV; label = 2, statusAngle = 0)
    updatePmu!(system14, device, analysisLAV; label = 6, statusAngle = 1)
    updatePmu!(system14, device, analysisLAV; label = 9, angle = analysis.voltage.angle[9], varianceAngle = 1e-5)
