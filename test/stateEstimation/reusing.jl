...
system14 = powerSystem(string(pathData, "case14test.m"))
@testset "Reusing Power System DC State Estimation" begin
    ############### Modified IEEE 14-bus Test Case ################
    dcModel!(system14)
    powerFlow = dcPowerFlow(system14)
    solve!(system14, powerFlow)
    power!(system14, powerFlow)
  
    ############### Measurements ################
    device = measurement()
  
    for (key, value) in system14.bus.label
        addWattmeter!(system14, device; bus = key, active = powerFlow.power.injection.active[value], noise = false)
        addPmu!(system14, device; bus = key, magnitude = 1.0, angle = powerFlow.voltage.angle[value], noise = false)
    end
    for (key, value) in system14.branch.label
        addWattmeter!(system14, device; from = key, active = powerFlow.power.from.active[value], noise = false)
        addWattmeter!(system14, device; to = key, active = powerFlow.power.to.active[value], noise = false) 
    end
    addWattmeter!(system14, device; bus = 2, active = rand(1)[])
    addWattmeter!(system14, device; from = 1, active = rand(1)[])
    addWattmeter!(system14, device; from = 1, active = rand(1)[])
    addWattmeter!(system14, device; to = 2, active = rand(1)[])
    addWattmeter!(system14, device; to = 2, active = rand(1)[])
    addPmu!(system14, device; bus = 2, magnitude = 1.0, angle = rand(1)[])
  
    ############### Solution ################
    updateBus!(system14; label = 2, conductance = 10)
    updateBus!(system14; label = 1, angle = -0.17)
    updateBranch!(system14; label = 1, reactance = 15, shiftAngle = 2.1, turnsRatio = 0.8)
    updateBranch!(system14; label = 2, status = 0)
    updateBranch!(system14; label = 18, status = 1)
    analysisCon = dcStateEstimation(system14, device)
    solve!(system14, analysisCon)
  
  
    ############### Update Power System ################
    updateBus!(system14; label = 2, conductance = 0)
    updateBus!(system14; label = 1, angle = 0.0)
    updateBranch!(system14; label = 1, reactance = 1, shiftAngle = 0.0, turnsRatio = 1.0)
    updateBranch!(system14; label = 2, status = 1)
    updateBranch!(system14; label = 18, status = 0)
    analysis = dcStateEstimation(system14, device)
  
    updateBus!(system14, analysis; label = 2, conductance = 10)
    updateBus!(system14, analysis; label = 1, angle = -0.17)
    updateBranch!(system14, analysis; label = 1, reactance = 15, shiftAngle = 2.1, turnsRatio = 0.8)
    updateBranch!(system14, analysis; label = 2, status = 0)
    updateBranch!(system14, analysis; label = 18, status = 1)
  
    solve!(system14, analysis)
  
    @test analysis.voltage.angle ≈ analysisCon.voltage.angle
end